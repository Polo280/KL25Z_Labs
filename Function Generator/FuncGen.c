...
static uint32_t CalculateTicks(uint64_t);

// Initial values for function generation
uint16_t value_index = 0;
uint8_t mode = SQUARE;
uint64_t update_ticks = 0;
uint64_t frequency = 1;   // Frequency in hertz

// Time aux
uint64_t blink_aux = 0, wave_aux = 0;
// UART buffer
char uart_rx[50];
char uart_tx[50];
// Systick pointer
...
int main(void) {
    ...
    while (1) {
        ...
        if(*systicks - wave_aux >= update_ticks){
        	value_index = (value_index >= WAVEFORM_LENGTH - 1)? 0 : value_index+1;
...
static void SineGenerate(uint64_t freq){
	...
	for (uint16_t i = 0; i < WAVEFORM_LENGTH; i++) {
		lookupTable[i] = 2047 * (sinf(fRadians * i) + 1);
	}
	uint32_t period_ticks = CalculateTicks(freq);
	update_ticks = period_ticks / WAVEFORM_LENGTH;
}
...
static void TriangularGenerate(uint64_t freq){
	double slope_delta = 4095.0 / ((double) WAVEFORM_LENGTH / 2.0);
	for (uint16_t i = 0; i < WAVEFORM_LENGTH; i++) {
		if(i <= WAVEFORM_LENGTH / 2){
...
static uint32_t CalculateTicks(uint64_t freq){
	 if (freq == 0) {
		return 0; // Handle the case where frequency is zero
	}
	double period = 1.0 / (double)freq;
	uint32_t ticks = (uint32_t)(period * 100000.0); // Assuming you are using TENS_MICROS as timebase
	return ticks;
}
